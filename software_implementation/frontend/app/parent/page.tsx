...
export default function ParentDashboard() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {/* My Children Card(s) */}
      {links.map((link) => {
        const childLatest = [...assessments].filter(a => a.patientId === link.patientId || a.patientName === link.patientName).sort((a, b) => {
          const tA = (a.createdAt as any)?.toMillis?.() || 0;
          const tB = (b.createdAt as any)?.toMillis?.() || 0;
          return tB - tA;
        })[0];
        const childRisk = childLatest ? childLatest.risk_level : 'Unknown';
        return (
          <motion.div
            key={link.id}
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{links.length > 1 ? link.patientName : 'My Child'}</h2>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${riskColors[childRisk] || riskColors.Unknown}`}>
                {childRisk}
              </span>
            </div>
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">{link.patientName}</h3>
            ...
            {link.professionalName && (
              <div className="border-t border-gray-100 dark:border-dark-deep pt-4">
                ...
                <div className="flex items-center justify-between">
                  <p className="text-gray-900 dark:text-white">{link.professionalName}</p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setShowContact(showContact === link.id ? null : link.id)}
            ...
            {showContact === link.id && (
              ...
              >
                <Link href="/parent/messages" className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-dark-bg rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-dark-deep">
                  ...
                  Send Message to {link.professionalName || 'Specialist'}
                </Link>
              </motion.div>
            )}
          </motion.div>
      ...
      {links.length === 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          className="p-6 rounded-2xl" style={{ backgroundColor: 'rgba(116, 179, 206, 0.1)', border: '1px solid rgba(116, 179, 206, 0.3)' }}>
          <h2 className="text-lg font-semibold mb-3" style={{ color: 'var(--foreground)' }}>Welcome to JASMINE 🎉</h2>
          <p className="text-sm mb-3" style={{ color: 'var(--text-muted)' }}>
            A professional will invite you to link your child&apos;s account. Once connected, you&apos;ll be able to:
          </p>
          <ol className="space-y-2 text-sm" style={{ color: 'var(--text-muted)' }}>
            <li><strong>1.</strong> View assessment results and screening scores</li>
            <li><strong>2.</strong> Read your doctor&apos;s notes and recommendations</li>
            <li><strong>3.</strong> Message your child&apos;s specialist directly</li>
            <li><strong>4.</strong> Track progress over time</li>
          </ol>
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <Link href="/parent/results" className="flex items-center gap-3 p-4 bg-white dark:bg-dark-surface rounded-xl border border-gray-200 dark:border-dark-deep hover:border-primary transition-colors">
          <FileText className="w-6 h-6 text-primary" />
          <span className="font-medium text-gray-900 dark:text-white">View Results</span>
        </Link>
        <Link href="/parent/messages" className="flex items-center gap-3 p-4 bg-white dark:bg-dark-surface rounded-xl border border-gray-200 dark:border-dark-deep hover:border-primary transition-colors">
