...
def _generate_feature_names() -> List[str]:
    ...
    try:
        ...
        dummy = np.zeros((10, NUM_KEYPOINTS, 3), dtype=np.float32)
        _, kin_names = extract_kinematic_features(dummy[:, :, :2], fps=15)
        _, stat_names = extract_all_features(dummy[:, :, :2], fps=15)
        _FEATURE_NAMES_CACHE = kin_names + stat_names
        return _FEATURE_NAMES_CACHE
    except Exception as e:
...
def extract_features_from_keypoints(keypoints: np.ndarray, fps: int = 15) -> tuple:
    ...
    kin_features, kin_names = extract_kinematic_features(coords_2d, fps=fps)
    stat_features, stat_names = extract_all_features(coords_2d, fps=fps)
    all_features = np.concatenate([kin_features, stat_features])
    feature_names = kin_names + stat_names
    ...
    return all_features, dl_sequence, feature_names
...
def get_explainability(models: Dict, features: np.ndarray,
                       predictions: Dict[str, float],
                       feature_names: List[str]) -> Dict:
    explain = {}

    # Feature importance from RF model (top 10)
    if 'rf' in models and feature_names:
        try:
            importance_dict = models['rf'].get_feature_importance(feature_names)
            top_features = sorted(importance_dict.items(), key=lambda x: x[1], reverse=True)[:10]
            explain['feature_importance'] = {name: round(score, 4) for name, score in top_features}
        except Exception as e:
            logger.error(f"Failed to get feature importance: {e}")

    if 'feature_importance' not in explain:
        explain['feature_importance'] = {}

    # Ensemble weights
    explain['ensemble_weights'] = load_ensemble_weights()

    # Model confidence (distance from 0.5, scaled to 0-1)
    explain['model_confidence'] = {}
    for model_type, prob in predictions.items():
        confidence = round(abs(prob - 0.5) * 2, 3)
        explain['model_confidence'][model_type] = confidence
...
def run_pipeline_from_file(video_path: str, fps: int = 15, max_frames: int = 300,
    ...
    try:
        ...
        features, dl_sequence, feature_names = extract_features_from_keypoints(keypoints, fps=fps)
        ...
        explainability = get_explainability(models, features, predictions, feature_names)
        ...
        result = {
            ...
            "ensemble_weights": explainability['ensemble_weights'],
            "feature_importance": explainability['feature_importance'],
            "model_confidence": explainability['model_confidence'],
...
async def predict_json(file: UploadFile = File(...)):
    ...
    try:
        ...
        features, dl_sequence, feature_names = extract_features_from_keypoints(keypoints)
        models = load_models()
        predictions = get_ensemble_prediction(models, features, dl_sequence)
